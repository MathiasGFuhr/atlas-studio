...
export function buildCandidateProposalPrompt(input: {
  ...
  return [
    ...
    `videoDuration: ${input.duration.toFixed(1)}`,
    `O usuário pediu até ${input.clipCount} Shorts de ~${input.requestedDuration}s (${input.durationMode}).`,
    `Escolha até ${input.clipCount} candidatos fortes e temporalmente distintos.`,
    `Tente chegar a ${input.clipCount}. Só devolva menos se realmente não houver outro trecho distinto com qualidade.`,
    'Não escolha só os melhores no mesmo clímax. Se dois cortes se sobrepõem demais, prefira o próximo melhor em outra região do vídeo.',
    'Uma pequena sobreposição é aceitável em vídeo curto (ex.: 00–30 e 26–56). Não devolva o mesmo trecho deslocado 2 segundos.',
    `Pode devolver até ${Math.max(input.clipCount + 6, input.clipCount * 3)} opções para o seletor local fazer backfill. A diversidade final é validada no Atlas.`,
    'Nunca duplique o mesmo start/end.',
    watch,
