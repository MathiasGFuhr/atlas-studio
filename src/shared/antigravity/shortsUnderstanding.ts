...
export function buildCandidateProposalPrompt(input: {
  ...
  return [
    ...
    `videoDuration: ${input.duration.toFixed(1)}`,
    `O usuário pediu ${input.clipCount} Shorts de ~${input.requestedDuration}s (${input.durationMode}). A quantidade final é obrigatória na camada local do Atlas.`,
    ...
    'Não escolha só os melhores no mesmo clímax. Sobreposição é aceitável em vídeo curto, desde que start/end não sejam idênticos.',
    'Uma sobreposição grande é aceitável quando o vídeo não cabe sem cruzar (ex.: 00–45, 06–51, 11.6–56.6). Não devolva o mesmo trecho copiado.',
    'A diversidade e o preenchimento da cota são validados no Atlas. Nunca duplique o mesmo start/end.',
    watch,
